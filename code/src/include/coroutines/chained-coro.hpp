...
namespace hashtable_coro
{
    ...
    public:
        ...
        using LookupType = LookupResult<Key, Payload>;
    ...
    public:
         ...
         */
        LookupType lookup(const Key &key) const
        {
            if (unlikely(key == Sentinel))
            {
                assert(false); // TODO(unknown): this must never happen in practice
            ...
            if (slot.key == key)
            {
                return LookupType{slot.key,slot.payload};
            }
            ...
            while (bucket != nullptr)
            {
                for (size_t i = 0; i < BucketSize; i++)
                {
                    ...
                    {
                        return LookupType{bucket->slots[i].key,bucket->slots[i].payload};
                    }
    ...
    public:
        explicit ChainedRMICoro(const size_t &capacity, const HashFn hashfn = HashFn())
            : hashfn(hashfn), reductionfn(ReductionFn(directory_address_count(capacity))), capacity(capacity),
              slots(directory_address_count(capacity)) {
            // TODO - assert that hash function is RMI
            // static_assert(std::is_base_of<rmi_coro::RMIHash<>, HashFn>::value, "HashFn must be rmi_coro::RMIHash");
        };

        ChainedRMICoro(ChainedRMICoro &&) noexcept = default;
        ...
        static forceinline std::string name()
        {
            return "chained_coro_" + hash_name() + "_" + reducer_name() + "_" + std::to_string(bucket_size());
        }
